...
function cuenta_guardar(pCatcuenta) {
  // Contiene todas las categorias de cuenta como objeto
  var objCuenta;
  ...
  cadCuenta = localStorage.getItem(item_cuenta);
  ...
  if (cadCuenta == null) {
    ...
    arrCuenta = JSON.parse(cadCuenta);
  }
  ...
  // Se busca si el registro a guardar ya existe dentro del arreglo
  posicion = arrCuenta.findIndex(registro=>registro.codigo == objCuenta.codigo);

  if (posicion == -1) {
    // Se trata de un registro nuevo

    // Se agrega el objeto actual dentro del arreglo
    arrCuenta.push(objCuenta);
  }
  else {
    // Se trata de un registro que ya existe dentro del arreglo

    // Se actualiza el valor del registro dentro del arreglo
    arrCuenta[posicion] = objCuenta;
  }

  // Se convierte el arreglo en una cadena de texto JSON
  cadCuenta = JSON.stringify(arrCuenta);

  // Se almacena el arreglo dentro del computador
  localStorage.setItem('cuenta', cadCuenta);
}
...
function cuenta_consultar() {
  // Se traen las cuentas almacenadas dentro del Local Storage
  cadCuenta = localStorage.getItem(item_categoria_gasto);
